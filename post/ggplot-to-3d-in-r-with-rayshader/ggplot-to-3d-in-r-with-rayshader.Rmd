...
summary: ''
authors: []
lastmod: '2019-09-24T19:30:48+02:00'
featured: no
image:
  caption: ''
  focal_point: ''
...
*In 3 minutes reading, You will learn how to turn your ggplot visualizations into amazing interactive 3D plots you can export or embed in HTML/Rmarkdown.*
...
knitr::include_graphics('post/img/rayshader_header.png')
```
...
knitr::include_graphics('post/img/rayshader_repo.PNG')
```
...
We usually want to start our pratical work drawing the main steps in our project and our principal goals. So in a general layer, we want to visualiza the average age. Firstly in a ggplot-color way, go one step further and make the plot 3D and end with an ani...

Comencemos nuestro ejercicio práctico, como casi siempre que queramos realizar algún proyecto, reflejando claramente lo que queremos conseguir. Lo que queremos crear es una visualización del mapa de España, donde se refleje la edad media de cada municipio. En un primer paso la edad media la codificaremos según el color del municipio, en una escala donde la edad media más elevada sea color X y la menos sea X. En un paso mñas allá, querremos un gráfico 3D, donde en lugar de color (o de manera añadida) querremos que la edad media referente a cada municipio se represente mediante relieves en el propio mapa.

Vayamos ahora a los pasos para conseguirlo.
...
[enlace](<http://www.ine.es/dyngs/INEbase/es/operacion.htm?c=Estadistica_C&cid=1254736177012&menu=resultados&secc=1254736195461&idp=1254734710990>). 
...
path <- "post/ggplot-to-3d-in-r-with-rayshader"
if (!require(pxR)) {
...
utils::download.file(url = "http://www.ine.es/pcaxisdl/t20/e245/p05/a2018/l0/00000006.px",
                     destfile = paste0(path,"/data/census_2018.px"))
...
tbl_census_2018 <- read.px(paste0(path,"/data/census_2018.px")) %>%              # Load & format
  as.tibble()
tbl_census_2018 %<>% 
...
unzip(temp, exdir = paste0(path,"/data/cities_gis"))          # Unzip in data/cities_gis
...
tlb_cities_gis <- readOGR(dsn = paste0(path,"/data/cities_gis/Municipios_ETRS89_30N.shp"),
...
e can see that these missing values represents just 1% of the data, so we are going to impute them with the previous costal pode info. Bet that you can easily improve this procedure but I consider it's prety good enought seeing the low NA ratio. 
...
plot_gg(plot_cities,multicore=TRUE,width=5,height=3,scale = 250)    # Plot_gg de rayshader
render_snapshot()
```
...
```{r, eval=FALSE}
render_movie(paste0(path,"/img/movie_spain.mp4"),frames = 720, fps=30,zoom=0.6,fov = 30)
```


<video style="display:block; margin: 0 auto;" controls width="700" height="600" class="center" autoplay loop muted playsinline>
  <source src="post/ggplot-to-3d-in-r-with-rayshader/img/movie_spain.mp4" type="video/mp4" />
</video>
<video width="320" height="240" controls>
  <source src="post/ggplot-to-3d-in-r-with-rayshader/img/movie_spain.mp4" type="video/mp4">
</video>
![](post/ggplot-to-3d-in-r-with-rayshader/img/movie_spain.gif)
