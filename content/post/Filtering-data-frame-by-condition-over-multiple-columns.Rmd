...
date: '2019-10-05'
slug: filtering-a-data-frame-by-condition-on-multiple-columns
output:
  blogdown::html_page: 
    highlight: tango
categories:
...
```
Here we have used the function ```all_vars``` in the predicate to explicit that the condition must be satisfied **in every feature**.
To be honest, for that purpose would have been easier to simply use ```iris %>% na.omit()```.
...
Also, there are some other fancy ways to manipulate data frames with the filter family. One trick is using ```vars(contains(...))``` to select the variables:
